...
export const BrowseDelete = ({
  setOpenbrowseDeleteModal,
  setBrowseData,
  id,
}) => {
  ...
  const handleDelete = () => {
    ...
    axios
      .delete(`${BaseUrl.url}browse/${id}?token=${token}&adminid=${adminid}`)
      .then((res) => {
        if (res.status === 200) {
          setOpenbrowseDeleteModal(false);
      ...
      >
        ...
        >
          <div class="relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
            <button
              onClick={() => setOpenbrowseDeleteModal(false)}
              type="button"
